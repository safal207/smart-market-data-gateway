...
A change to any workflow or validator must include a mutation/regression test that fails when the protection is removed. Review evidence never grants merge authority, and bots must never merge automatically.
...
2. bind the review request to the current exact head SHA and request another review after any later commit;
3. resolve every actionable finding or record why it is not applicable;
4. require the exact-head `Causal PR Gate`, repository CI, security scans, and applicable CodeQL jobs to pass;
5. confirm that no unresolved review thread remains;
6. let the repository owner make the final merge decision manually.

An unavailable or rate-limited reviewer must be reported honestly but does not permanently block a solo maintainer when another automated reviewer has completed review and the causal evidence is green. Automated review is decision support; the repository owner remains the only merge authority.
