...
describe("CandleAggregator", () => {
  it("builds deterministic OHLC candles and uses update count as volume fallback", () => {
    ...
    const candle = aggregator.getCandles("AAPL.US")[0]!;
  ...
  it("closes the prior bucket exactly at the boundary", () => {
    const aggregator = new CandleAggregator("5s");
    aggregator.ingest(
      makePoint(1, { quote: { providerTimestampMs: 4_999 } }),
    );
    const update = aggregator.ingest(
      makePoint(2, { quote: { providerTimestampMs: 5_000 } }),
    );
    expect(update.newlyCompleted).toHaveLength(1);
    expect(update.newlyCompleted[0]?.complete).toBe(true);
    expect(aggregator.getCandles("AAPL.US").map((candle) => candle.openTimeMs)).toEqual([
      0,
      5_000,
    ]);
  });
  ...
  it("rebuilds identically regardless of input ordering", () => {
    const points = [
      makePoint(1, { quote: { price: 100 } }),
      makePoint(2, { quote: { price: 105 } }),
      makePoint(3, { quote: { price: 99 } }),
    ];
