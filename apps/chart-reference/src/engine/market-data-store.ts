...
export class MarketDataStore {
  ...
  private connectionStatus: SourceStatus = Object.freeze({
    state: "idle",
    generation: 0,
    reconnectAttempt: 0,
  });
  ...
  constructor(options: MarketDataStoreOptions = {}) {
    ...
    this.eventRetentionPerSymbol = positiveInteger(
      options.eventRetentionPerSymbol ?? 10_000,
      "eventRetentionPerSymbol",
    );
    this.candleRetentionPerSymbol = positiveInteger(
      options.candleRetentionPerSymbol ?? 2_000,
      "candleRetentionPerSymbol",
    );
    this.logRetention = positiveInteger(options.logRetention ?? 500, "logRetention");
    this.integrityGuard = new TemporalIntegrityGuard({
      quarantineRetention: this.logRetention,
      ...options.integrity,
    });
  ...
  getSnapshot(): MarketDataStoreSnapshot {
  ...
  ingest(point: MarketDataPoint): IntegrityDecision {
    ...
    if (decision.outcome === "accepted") {
      let retained = this.eventsBySymbol.get(point.quote.symbol);
      if (retained == null) {
        retained = [];
        this.eventsBySymbol.set(point.quote.symbol, retained);
      }
      retained.push(point);
      trimStart(retained, this.eventRetentionPerSymbol);
      this.latestBySymbol.set(point.quote.symbol, point);
      this.aggregator.ingest(point);
      this.acceptedEntries.push(
        Object.freeze({
          quote: point.quote,
      ...
      const symbol = point.quote.symbol;
      this.duplicateCountsBySymbol.set(
        symbol,
  ...
  setTimeframe(timeframe: Timeframe): void {
    ...
    this.aggregator.rebuild(
      [...this.eventsBySymbol.values()].flatMap((points) => points),
    );
    this.publish();
  }
  ...
  attachSource(source: MarketDataSource): Unsubscribe {
    ...
    const unsubscribeEvents = source.subscribe((event) => {
      if (event.type === "market-data") this.ingest(event);
    });
  ...
  private createAggregator(timeframe: Timeframe): CandleAggregator {
    return new CandleAggregator(timeframe, {
      maxCandlesPerSymbol: this.candleRetentionPerSymbol,
    });
  }
  ...
  private buildSnapshot(): MarketDataStoreSnapshot {
    ...
    for (const [symbol, point] of [...this.latestBySymbol.entries()].sort(([left], [right]) =>
    ...
    for (const [symbol, count] of [...this.duplicateCountsBySymbol.entries()].sort(
      ([left], [right]) => left.localeCompare(right),
    )) {
      duplicateCounts[symbol] = count;
    }
    const candles: Record<string, readonly Candle[]> = {};
    for (const symbol of this.aggregator.getSymbols()) {
      candles[symbol] = this.aggregator.getCandles(symbol);
    }
