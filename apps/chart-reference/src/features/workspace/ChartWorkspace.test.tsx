...
vi.mock("../../chart", () => ({
  MarketChart: ({ symbol, timeframeLabel, emptyMessage }: { symbol: string; timeframeLabel: string; emptyMessage?: string }) => (
...
const model: ChartWorkspaceModel = {
  ...
  timeframe: "1m",
  timeframeOptions: [{ value: "5s", label: "5 seconds" }, { value: "1m", label: "1 minute" }],
  ...
  quote: {
    symbol: "AAPL.US", price: 215.42, bid: 215.4, ask: 215.44,
    provider: "synthetic-replay", providerTimestampMs: Date.UTC(2026, 0, 5, 14, 30),
    origin: "replay", sourceStale: false, ageMs: 0, direction: "up", stale: false,
  },
  candles: [{
    timeMs: Date.UTC(2026, 0, 5, 14, 30), open: 215.1, high: 215.7, low: 214.95,
    close: 215.42, updateCount: 38, activityValue: 38, activitySource: "updates",
  }],
  precision: 2,
...
function actions(): ChartWorkspaceActions {
  return {
    selectSymbol: vi.fn(), selectTimeframe: vi.fn(), selectTheme: vi.fn(),
    selectSourceMode: vi.fn(), setGatewayToken: vi.fn(), togglePaused: vi.fn(), reconnect: vi.fn(),
  };
}
...
describe("ChartWorkspace", () => {
  it("renders semantic quote, connection, summary, and attribution content", () => {
    ...
    expect(screen.getByRole("link", { name: /TradingView Lightweight Charts/i })).toHaveAttribute("href", "https://www.tradingview.com/");
    expect(screen.getByText(/Copyright © 2025 TradingView, Inc\./)).toBeVisible();
  });
  ...
  it("labels the local token as memory-only and forwards it without persistence", async () => {
    const handlers = actions();
    const user = userEvent.setup();
    render(<ChartWorkspace model={{ ...model, sourceMode: "gateway", gatewayToken: "local-token" }} actions={handlers} />);
  ...
  it("does not claim Live before the first gateway quote and times out to No data", () => {
    vi.useFakeTimers();
    render(<ChartWorkspace model={{
      ...model, sourceMode: "gateway", connectionState: "live", connectionDetail: "Socket opened.",
      lastUpdateMs: undefined, quote: null, candles: [],
    }} actions={actions()} />);
    ...
    expect(screen.getByText("No market data is available for this instrument and timeframe.")).toBeVisible();
