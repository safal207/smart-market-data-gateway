...
def test_candle_history_requires_entitlement_and_returns_server_series(test_settings) -> None:
    ...
    try:
        ...
        with TestClient(create_app(test_settings)) as client:
            ...
            future_end = client.get(
                "/v1/candles/AAPL",
                params={"end": (datetime.now(UTC) + timedelta(minutes=1)).isoformat()},
                headers={"Authorization": "Bearer dev-pro:bob"},
            )
            assert future_end.status_code == 422
            assert future_end.json()["detail"] == "end must not be in the future"
    finally:
        redis.flushdb()
        redis.close()
